<?php 
/**
* Plugin Name: Post and Categories Indexer
* Plugin URI: https://www.anaadisoft.com/rg-post-category-indexer
* Description: Generates a index of post and categories. 
* Version: 1.1.2
* Author: Brij Raj Singh
* Author URI: https://www.anaadisoft.com
...
if (!defined('RG_PCI_PLUGIN_NAME'))
    define('RG_PCI_PLUGIN_NAME', trim(dirname(plugin_basename(__FILE__)), '/'));

if (!defined('RG_PCI_PLUGIN_DIR'))
    define('RG_PCI_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . RG_PCI_PLUGIN_NAME);
    
function rgpci_index_generator($params){   
	...
	if(isset($params['category'])){
		...
		if($category_by_slug || $category_by_id){
            ...
            $rgpci_content = rgpci_get_content($category_object,$params);
  
			return $rgpci_content;
		}else{
...
function rgpci_get_content($category_object,$params){
	switch($params['display']){
		case 'list': $array_data = rgpci_generate_category_list_array($category_object->term_id,$params); break;
		case 'tree': $array_data = rgpci_generate_category_tree_array($category_object->term_id,$params); break;
	}	

	if(in_array("category_name",$params['show'])){
		$category_data = array();
		if(in_array("category_name",$params['links'])){
			$category_name = "<span class=\"rg-category\"><a href=\"".get_category_link($category_object->term_id)."\" class=\"rg-category-link\" title=\"".$category_object->name."\">".$category_object->name."</a></span>";
		}else{
			$category_name = "<span class=\"rg-category\">".$category_object->name."</span>";
		}		
	
		if(in_array("post_title",$params['show'])){
			if(!in_array($category_object->slug,$params['hide_category_posts'])){	
				$posts_data = rgpci_get_posts($category_object->term_id,$params);
				if($posts_data){
		...
		//ADDON FOR A CATERG_PCIORY WIHT NO CHILD CATEGORIES
	...
	return "<ul class=\"rg-ul-list rg-top-ul-list\">".rgpci_generate_tree($array_data)."</ul>";
}

function rgpci_generate_category_tree_array($category_id,$params){
	...
	foreach($subcategories as $subcategory){
		...
		if($show_category){
			if(in_array("subcategory_name",$params['links'])){
				$subcategory_name = "<span class=\"rg-category\"><a href=\"".get_category_link($subcategory->term_id)."\" class=\"rg-category-link\" title=\"".$subcategory->name."\">".$subcategory->name."</a></span>";
			}else{
				$subcategory_name = "<span class=\"rg-category\">".$subcategory->name."</span>";
			}
			...
			if(in_array("post_title",$params['show'])){
				if(!in_array($subcategory->slug,$params['hide_category_posts'])){	
					$posts_data = rgpci_get_posts($subcategory->term_id,$params);
			...
			$category_child_data = rgpci_generate_category_tree_array($subcategory->term_id,$params);
...
function rgpci_generate_category_list_array($category_id,$params){
...
function rgpci_generate_tree($array_data){
	$tree_data = "";
	foreach($array_data as $node){
		if(is_array($node)){
			$tree_data .= "<ul class=\"rg-ul-list\">".rgpci_generate_tree($node)."</ul>";
		}else{
			$tree_data .="<li class=\"rg-li-item\">".$node."</li>";
		}
	}
	return $tree_data;
}

function rgpci_get_posts($category_id,$params=null){
	$post_names = array();

	$posts_data = rgpci_get_category_posts($category_id,$params);
		...
		if(in_category($category_id, $post_data)){		
            ...
            if(!empty($params['postappend'])){
                ...
                if(in_array("post_excerpt",$postappend_array)){	
                    ...
                    $append.=$params['postappend_prefix']."<span class=\"rg-post-excerpt\">".trim($post_data->post_excerpt)."</span>";
                }                
                
                $postappend_tags = rgpci_find_tag_content($post_data->post_content,htmlspecialchars_decode($params['postappend']));
                if($postappend_tags){
            ...
            if(!empty($params['postappend_link'])){
                ...
                if(in_array("post_excerpt",$postappend_link_array)){
                    ...
                    $append_link.=$params['postappend_prefix']."<span class=\"rg-post_excerpt\">".trim($post_data->post_excerpt)."</span>";
                }			
                
                $postappend_link_tags = rgpci_find_tag_content($post_data->post_content,htmlspecialchars_decode($params['postappend_link']));
			...
			if(in_array("post_title",$params['links'])){
				$post_name = "<span class=\"rg-post-title\"><a href=\"".get_permalink($post_data->ID)."\" class=\"rg-post-title-link\" title=\"".$post_data->post_title."\">".$post_data->post_title.$append_link."</a></span>  ";
			}else{
				$post_name = "<span class=\"rg-post-title\">".$post_data->post_title.$append_link."</span>";
			}
			...
			if(isset($params['post_sections'])){
				...
				$post_tags = rgpci_find_tag_content($post_data->post_content,htmlspecialchars_decode($post_sections));
				if($post_tags){
...
function rgpci_get_category_posts($category_id,$params){
	$extra_param="";
...
function rgpci_find_tag_content($post_content,$search_query){
...
add_shortcode('rgpci', 'rgpci_index_generator');
